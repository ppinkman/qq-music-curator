...
class QQMusicAPI:
    ...
    CREATED_PLAYLISTS_URL = "https://c.y.qq.com/rsc/fcgi-bin/fcg_user_created_diss"
    PLAYLIST_MAP_URL = "https://c.y.qq.com/splcloud/fcgi-bin/fcg_musiclist_getmyfav.fcg"
    CREATE_PLAYLIST_URL = "https://c.y.qq.com/splcloud/fcgi-bin/create_playlist.fcg"
    ADD_TO_PLAYLIST_URL = "https://c.y.qq.com/splcloud/fcgi-bin/fcg_music_add2songdir.fcg"
    ...
    @staticmethod
    def _check_business_result(payload, operation, duplicate_code=None):
        if not isinstance(payload, dict):
            raise APIResponseError(f"{operation}返回了无效数据")
        try:
            code = int(payload.get("code", 0))
        except (TypeError, ValueError):
            code = -1
        if duplicate_code is not None and code == duplicate_code:
            raise APIResponseError(f"{operation}失败：歌单名称重复")
        if code in (1, 1000, 1001, 2000, 2001, 3000):
            raise AuthenticationError(f"{operation}失败：登录态无效（code={code}）")
        if code != 0:
            message = payload.get("msg") or payload.get("message") or "服务端拒绝请求"
            raise APIResponseError(f"{operation}失败（code={code}）：{message}")
        return payload

    def list_created_playlists(self):
        """Return playlists created by the current account, including dirid/tid."""
        payload = self._request_json(
            "GET",
            self.CREATED_PLAYLISTS_URL,
            params={
                "hostUin": 0,
                "hostuin": self.user_uin,
                "sin": 0,
                "size": 200,
                "g_tk": self.get_g_tk(),
        ...
        self._check_business_result(payload, "读取自建歌单")
        data = payload.get("data")
        items = data.get("disslist", []) if isinstance(data, dict) else []
        if not isinstance(items, list):
            raise APIResponseError("读取自建歌单失败：响应缺少 disslist")
        result = []
        for item in items:
            ...
            result.append(
                {
                    "name": str(item.get("diss_name") or ""),
                    "dirid": str(item.get("dirid") or ""),
                    "tid": str(item.get("tid") or ""),
                    "song_count": int(item.get("song_cnt") or 0),
                    "visible": bool(item.get("dir_show")),
                }
            )
        return result

    def get_playlist_song_mids(self, dirid):
        """Read the exact song MID set of one playlist owned by the account."""
        payload = self._request_json(
            "GET",
            self.PLAYLIST_MAP_URL,
            params={
                "dirid": str(dirid),
                "dirinfo": 1,
                "g_tk": self.get_g_tk(),
        ...
        self._check_business_result(payload, "读取歌单歌曲")
        mapping = payload.get("mapmid", {})
        if isinstance(mapping, dict):
            return {str(mid) for mid, enabled in mapping.items() if enabled}
        if isinstance(mapping, list):
            return {str(mid) for mid in mapping if mid}
        raise APIResponseError("读取歌单歌曲失败：响应缺少 mapmid")

    def create_playlist(self, name):
        """Create a playlist and return its account-local dirid."""
        name = str(name).strip()
        if not name:
        ...
        data = self._module_data(payload, "req_0")
        result = data.get("result") if isinstance(data, dict) else None
        dirid = str(result.get("dirId") or "") if isinstance(result, dict) else ""
        if not dirid:
            raise APIResponseError("创建歌单成功，但响应中没有 dirid")
        return dirid

    def edit_playlist_name(self, dirid, name):
        """Rename a playlist created by this importer using the current web API."""
        body = {
            "comm": {
                "ct": 24,
                "cv": 0,
        ...
        self._module_data(payload, "req_0")
        return True

    def add_songs_to_playlist(self, dirid, songs):
        """Add up to 50 {id, type, mid} song references to an owned playlist."""
        unique = []
        seen_ids = set()
        for song in songs:
            if not isinstance(song, dict) or not song.get("id"):
                raise ValueError("添加歌曲需要数字歌曲 id")
            song_id = int(song["id"])
            ...
            unique.append(
                {
                    "songId": song_id,
                    "songType": int(song.get("type") or 0),
                }
            )
        if not unique:
            return 0
        if len(unique) > 50:
            raise ValueError("单次最多添加 50 首歌曲")
        body = {
            "comm": {
                "ct": 24,
                "cv": 0,
            ...
            "req_0": {
                ...
                "method": "AddSonglist",
                "param": {"dirId": int(dirid), "v_songInfo": unique},
            },
        }
        payload = self._request_json(
            "POST",
        ...
        if ret_code not in (0, None):
            message = data.get("msg") or "服务端拒绝请求"
            raise APIResponseError(f"添加歌曲失败（retCode={ret_code}）：{message}")
        return len(unique)

    def remove_songs_from_playlist(self, dirid, songs):
        """Remove explicitly identified songs; used only for importer-owned cleanup."""
        song_info = []
        for song in songs:
            if not isinstance(song, dict) or not song.get("id"):
                raise ValueError("移除歌曲需要数字歌曲 id")
            song_info.append(
                {
                    "songId": int(song["id"]),
                    "songType": int(song.get("type") or 0),
        ...
        body = {
            ...
            "req_0": {
                "module": "music.musicasset.PlaylistDetailWrite",
                "method": "DelSonglist",
                "param": {"dirId": int(dirid), "v_songInfo": song_info},
            },
        }
        payload = self._request_json("POST", self.MUSICU_URL, json=body)
        data = self._module_data(payload, "req_0")
        ...
        if ret_code not in (0, None):
            message = data.get("msg") or "服务端拒绝请求"
            raise APIResponseError(f"移除歌曲失败（retCode={ret_code}）：{message}")
